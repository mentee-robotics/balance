...
class Brain:
    def __init__(self, cfg):
        ...
        logger.info(f"{self.init_pos}")
        # self.action_dict: Dict[str, float]
        # for i in self.init_pos:
        #     self.action_dict.set(i.key, i.value)
        self.action_dict = self.init_pos
        logger.info(f"{self.action_dict}")
        self.rolls = ["left_ankle_roll", "right_ankle_roll",  "left_hip_roll", "right_hip_roll"]
        for i in self.rolls:
            self.action_dict["kp_"+i] =  0.0
        for i in self.rolls:
            self.action_dict["kd_"+i] = 0.0
    ...
    def call_step(self):
        ...
        # print(self.action_dict)self.action_dict.set("kp_" + "left_ankle_roll", 50.0)
        pass
